...
def main(argc, argv):
    ...
    for result in p.imap_unordered(
        ...
        if result.returncode != wanted_returncode:
            ...
            if (result.returncode == 5 and args.crash_only is False) or \
               (result.returncode == 5 and args.crash_only):
            ...
            if args.crash_only is False:
                logging.debug(
                    ...
                    result.path,
                    'hang' if result.returncode == 6 else 'crash'
                )
            else:
                logging.debug(
                    '[x] The input file %s triggered a %s, skipping it.',
                    result.path,
                    'hang' if result.returncode == 6 else 'non crash'
                )
