...
char **Debugger::GetEnvp() {
  ...
  char **envp_ = (char**)malloc(sizeof(char*)*(envp_size+1));
  for (int i = 0; i < environ_size; ++i) {
    envp_[i] = (char*)malloc(strlen(environ[i])+1);
    strcpy(envp_[i], environ[i]);
  }

  if (gmalloc_mode) {
    envp_[envp_size-1] = (char*)malloc(strlen(GMALLOC_ENV_CONFIG)+1);
    strcpy(envp_[envp_size-1], GMALLOC_ENV_CONFIG);
  }
  envp_[envp_size] = NULL;
...
void Debugger::StartProcess(int argc, char **argv) {
  ...
  if (status != 0) {
    ...
    /*pipe(out);
    posix_spawn_file_actions_adddup2(&attr, out[1], STDOUT_FILENO);
    posix_spawn_file_actions_addclose(&attr, out[0]);*/
