...
u8 schedule = 0;               /* Power schedule (default: EXPLORE)   */
enum {
...
void mark_as_det_done(struct queue_entry* q) {
	...
	fd = _open(fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
...
void write_bitmap(void) {
	...
	fd = _open(fname, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0600);
...
void update_bitmap_score(struct queue_entry* q) {
	...
	if (unlikely(schedule >= FAST && schedule <= RARE))
		fuzz_p2 = next_p2(q->n_fuzz);
	else
		fuzz_p2 = q->fuzz_level;

	if (unlikely(schedule >= RARE)) {
	...
	for (i = 0; i < MAP_SIZE; i++)
		...
		if (trace_bits[i]) {
			...
			if (top_rated[i]) {
				...
				if (unlikely(schedule >= FAST && schedule <= RARE))
					top_rated_fuzz_p2 = next_p2(top_rated[i]->n_fuzz);
				else
			...
			if (!q->trace_mini) {
				q->trace_mini = (u8*)ck_alloc(MAP_SIZE >> 3);
...
void read_testcases(void) {
	...
	nl_cnt = scandir_sorted(in_dir, &nl);
...
void load_extras_file(u8* fname, u32* min_len, u32* max_len,
	...
	while ((lptr = fgets(buf, MAX_LINE, f))) {
		...
		*rptr = 0;
		...
		wptr = extras[extras_cnt].data = (u8*)ck_alloc(rptr - lptr);
...
static void load_extras(u8* dir) {
	...
	do {
		...
		extras = (struct extra_data*)ck_realloc_block((u8*)extras, (extras_cnt + 1) *
			sizeof(struct extra_data));

		extras[extras_cnt].data = (u8*)ck_alloc(fdata.nFileSizeLow);
...
void maybe_add_auto(u8* mem, u32 len) {
	...
	if (a_extras_cnt < MAX_AUTO_EXTRAS) {
		...
		a_extras[i].data = (u8*)ck_memdup(mem, len);
		a_extras[i].len = len;
...
u8 calibrate_case(int argc, char** argv, struct queue_entry* q, u8* use_mem,
	u32 handicap, u8 from_queue)
{
	...
	for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
		...
		fault = RunTarget(argc, argv, 0, old_tmout);
		cook_coverage();
...
void perform_dry_run(int argc, char** argv) {
	...
	while (q) {
		...
		fd = _open(q->fname, O_RDONLY | O_BINARY);
		if (fd < 0) PFATAL("Unable to open '%s'", q->fname);

		use_mem = (u8*)ck_alloc_nozero(q->len);

		if (_read(fd, use_mem, q->len) != q->len)
...
void link_or_copy(u8* old_path, u8* new_path) {
	...
	sfd = _open(old_path, O_RDONLY | O_BINARY);

	if (sfd < 0) PFATAL("Unable to open '%s'", old_path);

	dfd = _open(new_path, O_WRONLY | O_BINARY | O_CREAT | O_EXCL, 0600);
	if (dfd < 0) PFATAL("Unable to create '%s'", new_path);

	tmp = (u8*)ck_alloc(64 * 1024);

	while ((i = _read(sfd, tmp, 64 * 1024)) > 0)
...
u8 save_if_interesting(int argc, char** argv, void* mem, u32 len, u8 fault) {
	...
	switch (fault) {
		...
		case DEBUGGER_CRASHED:
			...
			fn = alloc_printf("%s\\crashes\\id_%06llu,sig_%02u,%s", out_dir, unique_crashes,
...
u8 delete_files(u8* path, u8* prefix) {
	...
	do {
		if (fd.cFileName[0] != '.' && (!prefix ||
			...
			if (_unlink(fname)) PFATAL("Unable to delete '%s'", fname);
			ck_free(fname);
...
void nuke_resume_dir(void) {
	...
	if (_rmdir(fn) && errno != ENOENT) goto dir_cleanup_failed;
	ck_free(fn);
...
void maybe_delete_out_dir(void) {
	...
	if (in_place_resume && _rmdir(fn)) {
		...
		u8* nfn = alloc_printf("%s.%04u-%02u-%02u-%02u_%02u_%02u", fn,
	...
	if (_unlink(fn) && errno != ENOENT) goto dir_cleanup_failed;
	ck_free(fn);
...
#define SP20    SP10 SP10
	...
	if (!dumb_mode && (last_path_time || resuming_fuzz || queue_cycle == 1 ||
		in_bitmap || crash_mode == DEBUGGER_CRASHED)) {
...
u8 trim_case(int argc, char** argv, struct queue_entry* q, u8* in_buf) {
	...
	if (needs_write) {
		...
		_unlink(q->fname); /* ignore errors */
		
		fd = _open(q->fname, O_WRONLY | O_BINARY | O_CREAT | O_EXCL, 0600);
		if (fd < 0) {
			SafeTerminateProcess();
			_unlink(q->fname); /* ignore errors */
			fd = _open(q->fname, O_WRONLY | O_BINARY | O_CREAT | O_EXCL, 0600);
			if (fd < 0) {
				if (errno == EEXIST) {
					fd = open(q->fname, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0600);
					if (fd < 0) {
...
/*
u8* g_backup_buf[10000] = {0};
u32 g_backup_idx = 0;
*/
u8 common_fuzz_stuff(int argc, char** argv, u8* out_buf, u32 len) {
	/*
	u8* x = g_backup_buf[g_backup_idx];
	if (x) {
		ck_free(x);
	}
	g_backup_buf[g_backup_idx] = (u8*)ck_alloc(len);
	memcpy(g_backup_buf[g_backup_idx], out_buf, len);
	g_backup_idx = (g_backup_idx + 1) % 10000;
	*/
...
u32 calculate_score(struct queue_entry* q) {
	...
	case MMOPT:
		...
		*/
		...
		if (max_depth - q->depth < 5) { perf_score *= 2; }
	...
	case RARE:
		...
		// reduce its value
		perf_score *=
			(1 - (double)((double)q->n_fuzz / (double)total_execs));
...
u8 fuzz_one_original(int argc, char** argv) {
	...
	if (pending_favored) {
		...
		if (((queue_cur->fuzz_level > 0 || queue_cur->was_fuzzed > 0) || !queue_cur->favored) &&
	...
	fd = _open(queue_cur->fname, O_RDONLY | O_BINARY);
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1)
	...
	eff_map = (u8*)ck_alloc(EFF_ALEN(len));
	eff_map[0] = 1;
...
havoc_stage:
	...
	for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
		...
		for (i = 0; i < use_stacking; i++) {
			...
			case 13:
				...
				if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
					...
					new_buf = (u8*)ck_alloc_nozero(temp_len + clone_len);
			...
			case 16: {
				...
				if (!extras_cnt || (a_extras_cnt && UR(2))) {
					...
					new_buf = (u8*)ck_alloc_nozero(temp_len + extra_len);
		...
		if (temp_len < len) out_buf = (u8*)ck_realloc(out_buf, len);
		temp_len = len;
		memcpy(out_buf, in_buf, len);
...
retry_splicing:
	...
	if (use_splicing && splice_cycle++ < SPLICE_CYCLES &&
		...
		fd = _open(target->fname, O_RDONLY | O_BINARY);

		if (fd < 0) PFATAL("Unable to open '%s'", target->fname);

		new_buf = (u8*)ck_alloc_nozero(target->len);
...
u8 pilot_fuzzing(int argc, char** argv) {
...
u8 core_fuzzing(int argc, char** argv) {
	int i;
	...
	cur_ms_lv = get_cur_time();
	if (!(key_puppet == 0 &&
		((cur_ms_lv - last_path_time < (u32)limit_time_puppet) ||
		(last_crash_time != 0 &&
			cur_ms_lv - last_crash_time < (u32)limit_time_puppet) ||
			last_path_time == 0))) {
...
function is a tad too long... returns 0 if fuzzed successfully, 1 if
skipped or bailed out. */
u8 fuzz_one(int argc, char** argv) {
	int key_val_lv_1 = 0, key_val_lv_2 = 0;
	// if limit_time_sig == -1 then both are run after each other

	if (limit_time_sig <= 0) { key_val_lv_1 = fuzz_one_original(argc, argv); }

	if (limit_time_sig != 0) {

		if (key_module == 0) {

			key_val_lv_2 = pilot_fuzzing(argc, argv);

		}
		else if (key_module == 1) {
			key_val_lv_2 = core_fuzzing(argc, argv);
...
void sync_fuzzers(int argc, char** argv) {
	...
	do {
		...
		id_fd = _open(qd_synced_path, O_RDWR | O_BINARY | O_CREAT, 0600);

		if (id_fd < 0) PFATAL("Unable to create '%s'", qd_synced_path);

		if (_read(id_fd, &min_accept, sizeof(u32)) > 0)
			_lseek(id_fd, 0, SEEK_SET);
...
void setup_dirs_fds(void) {
	...
	if (sync_id && _mkdir(sync_dir) && errno != EEXIST)
	...
	if (_mkdir(tmp)) PFATAL("Unable to create '%s'", tmp);
	ck_free(tmp);
	...
	fd = _open(tmp, O_WRONLY | O_BINARY | O_CREAT | O_EXCL, 0600);
	if (fd < 0) PFATAL("Unable to create '%s'", tmp);
	ck_free(tmp);

	plot_file = _fdopen(fd, "w");
	if (!plot_file) PFATAL("fd_open() failed");
...
void setup_stdio_file(void) {
	...
	out_fd = _open(fn, O_RDWR | O_BINARY | O_CREAT | O_EXCL, 0600);
...
int main(int argc, char **argv){
	...
	if (optarg) {
		if (!_stricmp(optarg, "explore")) {
			schedule = EXPLORE;
		}
		else if (!_stricmp(optarg, "exploit")) {
			schedule = EXPLOIT;
		}
		else if (!_stricmp(optarg, "fast")) {
			schedule = FAST;
		}
		else if (!_stricmp(optarg, "coe")) {
			schedule = COE;
		}
		
		else if (!_stricmp(optarg, "lin")) {
			schedule = LIN;
		}
		else if (!_stricmp(optarg, "quad")) {
			schedule = QUAD;
		}
		else if (!_stricmp(optarg, "rare")) {

			schedule = RARE;

		}
		else if (!_stricmp(optarg, "mopt") || !_stricmp(optarg, "mmopt")) {
	...
	/* MOpt mode */
	optarg = NULL;
	optarg = GetOption("-L", argc, argv);
	if (optarg) {
		havoc_max_mult = HAVOC_MAX_MULT_MOPT;
		if (sscanf(optarg, "%llu", &limit_time_puppet) < 1) FATAL("Bad syntax used for -L");

		if (limit_time_puppet == -1) {// L -1

			limit_time_sig = -1;
			limit_time_puppet = 0;

		}
		else if (limit_time_puppet < 0) {
		...
		if (g_now > g_max) { g_now = 0; }
		w_now = (w_init - w_end) * (g_max - g_now) /
			(g_max)+
			w_end;
		...
		for (tmp_swarm = 0; tmp_swarm < swarm_num; ++tmp_swarm) {
			...
			for (i = 0; i < operator_num; ++i) {
				...
				x_now[tmp_swarm][i] =
					x_now[tmp_swarm][i] / total_puppet_temp;
			...
			for (i = 0; i < operator_num; ++i) {
				...
				v_now[tmp_swarm][i] =
					...
					RAND_C *
					(L_best[tmp_swarm][i] - x_now[tmp_swarm][i]) +
